...
def count_word(textdata):
    ...
    for idx in range(len(textdata)):
        ...
        if type(textdata[idx]) == str:
            ...
            # 한글빼고 다 제거
            sent_text = [re.sub("[^가-힣]",' ',text) for text in sent_text[2:]]
            sent_text = [re.sub("\ +",' ',text) for text in sent_text]
            sent_text = [re.sub("이사의 경영진단 및 분석의견",'',text) for text in sent_text]
            # 공백이 2개 이상일때
            sent_text = [re.sub("  +",' ',text) for text in sent_text]
...
if __name__ == "__main__":
    ...
    # 데이터 합치기
    txt_df = []
    for tmp in process_text:
        txt_df.append(' '.join(tmp))
    txt_df = pd.DataFrame(txt_df)
    df = DataFrame(word_cnt)
    pos_df = DataFrame(pos)
    neg_df = DataFrame(neg)

    data2 = pd.concat([txt_df, df, pos_df, neg_df], axis=1)
    data2.columns = ["ko_text","단어수","긍정단어수","부정단어수"]
